...
// ── Build a progress snapshot for AI memory ──────────────────────────────────
function buildProgressSnapshot(curriculum, progress, weeklyHours, focus) {
  const completed = curriculum.filter(i=>(progress[i.id]?.percentComplete||0)>=100)
    .map(i=>i.id).join(", ") || "none";
  const inProgress = curriculum
    .filter(i=>{const p=progress[i.id]?.percentComplete||0; return p>0&&p<100;})
    .map(i=>`${i.id}(${progress[i.id].percentComplete}%)`)
    .join(", ") || "none";
  const totalH = curriculum.reduce((s,i)=>s+(progress[i.id]?.hoursSpent||0),0);
  const recentAvg = weeklyHours.slice(0,4).length>0
    ? (weeklyHours.slice(0,4).reduce((s,w)=>s+(w.realH||0),0)/weeklyHours.slice(0,4).length).toFixed(1)
    : "0";
  const date = new Date().toLocaleDateString("en-CA");
  return `[SNAPSHOT ${date}] totalRealH=${totalH.toFixed(0)} | avgWeekly=${recentAvg}h | focus=${[...(focus.courses||[]),...(focus.books||[])].join(",")} | completed=${completed} | inProgress=${inProgress}`;
}

// ── Append snapshot to profile, keep last 4 ──────────────────────────────────
function appendSnapshotToProfile(profile, snapshot) {
  const MARKER = "--- PROGRESS SNAPSHOTS ---";
  const lines = profile.includes(MARKER)
    ? profile.split(MARKER)[0].trimEnd()
    : profile.trimEnd();
  const existing = profile.includes(MARKER)
    ? profile.split(MARKER)[1].trim().split("\n").filter(Boolean)
    : [];
  const updated = [snapshot, ...existing].slice(0, 4);
  return `${lines}\n\n${MARKER}\n${updated.join("\n")}`;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const now = new Date();
    if (now.getDay()===1 && now.getHours()>=7 && !(weekPlan?.weekStart===getMonday()))
      setTimeout(() => runFullCheckin(true), 1500);
    if (now.getDay()===0 && !weeklySummary)
  ...
  const currentStreak = (()=>{
    let s=0; for (let i=0;i<weeklyHours.length;i++){if(weeklyHours[i].realH>=WEEKLY_TARGET)s++;else break;} return s;
  })();
  const longestStreak = (()=>{
    let max=0,cur=0;
  ...
  const genreBalance = (()=>{
    const map={};
    CURRICULUM.forEach(i=>{const p=getP(i.id);if(p.hoursSpent>0) map[i.genre]=(map[i.genre]||0)+(p.hoursSpent||0);});
  ...
  const buildAIContext = () => {
    const recentHistory = weekLogs.slice(0,4).map((l,i)=>
      `WEEK ${i+1} AGO (${l.date}): ${(l.hoursLogged||0).toFixed(1)}h real. Note:"${l.note||"none"}". Assessment:"${l.assessment||""}". Focus:${l.focusBefore?.join(",")||"unknown"}.`
    ).join("\n");
    ...
    const touchedAndFocus = CURRICULUM
      ...
      .map(i=>{
        ...
        return buildItemContext(i,p)+` | momentum=${momentum} (${recentH.toFixed(1)}h in last 2wk)`;
      }).join("\n");
    ...
    const arcPosition = `${arcYear}. ${totalDoneH.toFixed(0)}h real total. Completed genres: ${completedGenres.join(",")||"none"}. Active: ${inProgressGenres.join(",")||"none"}.`;
  ...
  const runFullCheckin = async (auto=false) => {
    if (!navigator.onLine) { enqueue("checkin",{auto}); setOfflineQueue(loadQueue()); toast_("Offline — queued"); return; }
...
- Remaining: ${effectiveWkRem.toFixed(2)}h across ${effectiveDLeft} day(s): ${remainingDayNames.join(", ")}.
...
FOCUS (${focus.manual?"MANUAL":"AI proposed"}): ${focusIds.join(", ")}
...
Respond ONLY as JSON — do NOT include a "focus" field on individual items:
{"assessment":"2 sentences max","insight":"1 sentence","nextMilestone":"1 sentence","focusProposal":{"courses":["A1"],"books":["B34","B99"],"reasoning":"1 sentence"},"days":[{"day":"Mon","totalDayRealH":3,"items":[{"id":"A1","realHours":1.5,"contentHours":0.75,"targetPct":44}]}],"totalPlannedHours":${effectiveWkRem.toFixed(2)}}`;
    ...
    try {
      ...
      if (Math.abs(drift)>=0.05 && validatedDays.length>0) {
        ...
        validatedDays[validatedDays.length-1] = {
          ...last, totalDayRealH:newDayH,
      ...
      const plan = {
        weekStart:getMonday(), generatedAt:new Date().toISOString(),
      ...
      // ── Auto-update profile with progress snapshot ──
      const snapshot = buildProgressSnapshot(CURRICULUM, progress, weeklyHours, focus);
      setProfile(prev => appendSnapshotToProfile(prev, snapshot));
...
Respond ONLY as JSON — do NOT include a "focus" field on individual items:
{"days":[{"day":"Tue","totalDayRealH":3,"items":[{"id":"A1","realHours":1.5,"contentHours":0.75,"targetPct":44}]}],"totalPlannedHours":${freshWkRem.toFixed(2)},"note":"1 sentence what changed","focusProposal":{"courses":["A1"],"books":["B34","B99"],"reasoning":"1 sentence"}}`;
  ...
  const markItemComplete = async (item) => {
    ...
    if (contentRemaining>0) {
      ...
      const monDate = new Date(getMonday()), sunDate = new Date(monDate);
  ...
  const runBonusSuggestions = async () => {
    ...
    const prompt = `Learner hit 20h target. Suggest 1-2 bonus sessions. ONLY JSON.
...
{"items":[{"id":"A1","realHours":1.5,"contentHours":0.75}],"note":"one sentence"}`;
  ...
  const runSundaySummary = async () => {
    ...
    const thisWeekItems = CURRICULUM
      ...
      .map(i=>{
        const p = getP(i.id);
        const wH = (p.sessions||[]).filter(s=>new Date(s.date)>=new Date(getMonday())).reduce((s,x)=>s+(x.studyHours||0),0);
        return `${i.id} "${i.name}": ${wH.toFixed(1)}h this week, now ${p.percentComplete}%`;
      }).join("\n");
    const stalledItems = CURRICULUM
      .filter(i=>{const p=getP(i.id);if(p.percentComplete>=100||p.percentComplete===0)return false;
        return !(p.sessions||[]).some(s=>new Date(s.date)>=new Date(Date.now()-14*24*60*60*1000));})
      .map(i=>`${i.id} stalled at ${getP(i.id).percentComplete}%`).join("\n");
    const prompt = `Write a short Sunday review (3-4 sentences). Be specific. No fluff.
Weekly hours: ${weekH.toFixed(1)}h / ${WEEKLY_TARGET}h. ${arcPosition}
...
Then 3-4 sentences of sharp context for Monday's AI plan. No labels or headers.`;
    try {
      const raw = await callAI(prompt, 500, "claude-sonnet-4-6");
      const parts = raw.split("---MONDAY_SEED---");
  ...
  const submitLog = (quickRealH=null, quickContentH=null) => {
    ...
    const contentH = isQuick ? quickContentH
      : (logForm.courseHours ? parseFloat(logForm.courseHours) : realToContent(logging, realH));
  ...
  const saveEditSession = () => {
    ...
    setWeek(w=>({...w, hoursLogged:Math.max(0,(w.hoursLogged||0)+(newRealH-(old.studyHours||0)))}));
  ...
  const addCustomItem = () => {
    ...
    const existingCustom = customItems.length;
    const prefix = type==="course" ? "C" : "D";
    const id = `${prefix}${existingCustom+1}`;
    const item = {id, name:name.trim(), hours:parseFloat(hours), type, section, genre:genre.trim(), custom:true};
  ...
  const doExport = () => {
    const data = {progress, week, focus, weekLogs, profile, weekPlan, weeklyHours, customItems};
  ...
  const doClearAll = () => {
    ...
    [SK_P,SK_W,SK_F,SK_LOG,SK_PROFILE,SK_PLAN,SK_QUEUE,SK_WEEKLY_HOURS,"tp_bonus1","tp_monday_seed","tp_last_export",SK_CUSTOM]
      .forEach(k=>localStorage.removeItem(k));
    setProgress({}); setWeek({weekStart:getMonday(),hoursLogged:0});
    setFocus({courses:["A1"],books:["B99","B34"]}); setWeekLogs([]);
    setProfile(DEFAULT_PROFILE); setWeekPlan(null); setWeeklyHours([]);
    setBonusItems([]); setOfflineQueue([]); setWeeklySummary(null); setCustomItems([]);
      ...
      <div style={{padding:"16px 14px"}}>
          ...
          {weekH>=WEEKLY_TARGET && <div>
            <Card style={{padding:"13px 14px",marginBottom:10,border:`1px solid ${T.green}15`}}>
              ...
              {bonusItems?.items?.length>0 && <div>
                {bonusItems.note && <div style={{fontSize:11,color:T.textMid,marginBottom:10,fontStyle:"italic",lineHeight:1.5}}>{bonusItems.note}</div>}
        ...
        {view==="week" && <div>
          ...
          {planIsFromThisWeek&&weekPlan.days && <Card style={{padding:"13px 14px",marginBottom:12}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      ...
                      <div style={{flexShrink:0,textAlign:"right"}}>
                        {isComplete
                          ...
                          : wasLogged
                            ? <div>
                                <div style={{fontSize:13,fontWeight:800,color:T.yellow,textShadow:shadow.glow(T.yellow)}}>{remainingH}h</div>
                                <div style={{fontSize:9,color:T.textDim}}>remaining of {it.realHours}h</div>
                              </div>
                            : <div>
                                <div style={{fontSize:13,fontWeight:800,color:T.blue,textShadow:shadow.glow(T.blue)}}>{it.realHours}h</div>
                                {it.contentHours&&f?.type==="course"&&<div style={{fontSize:9,color:T.textDim}}>{it.contentHours}h content</div>}
                              </div>}
                      </div>
                    </div>
            ...
            {weekH>=WEEKLY_TARGET&&<div style={{textAlign:"center",padding:"12px 0 4px",
              fontSize:11,color:T.green,fontWeight:700,letterSpacing:0.3,textShadow:shadow.glow(T.green)}}>
            ...
            return <Card key={item.id} accent={c} style={{marginBottom:10,padding:"13px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:15,fontWeight:800,color:c,textShadow:`0 0 10px ${c}40`}}>{p.percentComplete}%</div>
        ...
        {view==="ai" && <div>
          ...
          <Card style={{padding:"13px 14px",marginBottom:10}}>
            ...
            {!editProfile&&<div style={{fontSize:10,color:T.textDim,marginTop:6,lineHeight:1.5}}>
              {profile.includes("--- PROGRESS SNAPSHOTS ---")
                ? `Profile active · ${profile.split("--- PROGRESS SNAPSHOTS ---")[1].trim().split("\n").filter(Boolean).length} progress snapshot(s) stored`
                : "Profile active · no snapshots yet — run check-in to capture progress"}
            </div>}
          </Card>
          ...
          {(weeklySummary||summaryLoading) && <Card accent={T.yellow} style={{padding:"13px 14px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:9,color:T.yellow,textTransform:"uppercase",letterSpacing:1.5,fontWeight:700}}>Sunday Review</div>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>applyFocusProposal(aiResult.focusProposal)}
                  style={{flex:2,background:"#0a180a",border:`1px solid ${T.green}30`,color:T.green,
                    ...
                    boxShadow:`0 0 16px ${T.green}15`}}>Apply New Focus ✓</button>
              </div>
            </Card>}
          </div>}
        </div>}
        ...
        {view==="arc" && <div>
          <Card style={{marginBottom:16,padding:16}}>
            ...
            <div style={{background:T.surface0,borderRadius:12,padding:"12px 14px",marginBottom:12,border:`1px solid ${T.border}`}}>
              ...
              <div style={{display:"flex",alignItems:"flex-end",gap:3,height:56}}>
                {chartWeeks.map((w,i)=>{
                  ...
                  const isTarget = w.h>=WEEKLY_TARGET, isCurrent = i===11;
            ...
            <div style={{background:T.surface0,borderRadius:12,padding:"12px 14px",marginBottom:12,
              border:`1px solid ${T.border}`,boxShadow:shadow.card}}>
              <div style={{fontSize:9,color:T.textDim,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Real Study Hours Logged</div>
      ...
      {showSettings && <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",
        ...
        <div style={{background:T.surface1,minHeight:"100%",padding:"0 0 40px"}}>
          <div style={{position:"sticky",top:0,background:T.surface1,
            ...
            <div style={{fontSize:16,fontWeight:800,letterSpacing:-0.3}}>Settings</div>
          ...
          <div style={{padding:"16px 16px 0"}}>
            ...
            <div style={{fontSize:9,color:T.textDim,letterSpacing:1.5,textTransform:"uppercase",
              marginBottom:10,fontWeight:700}}>Data Backup</div>
            ...
            {/* Add Custom Item */}
            <div style={{fontSize:9,color:T.textDim,letterSpacing:1.5,textTransform:"uppercase",
              marginBottom:10,fontWeight:700}}>Add Custom Course or Book</div>
            <Card style={{padding:"13px 14px",marginBottom:12}}>
            ...
            {customItems.length>0 && <>
              <div style={{fontSize:9,color:T.textDim,letterSpacing:1.5,textTransform:"uppercase",
                marginBottom:10,fontWeight:700}}>Custom Items ({customItems.length})</div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{flex:1,minWidth:0,paddingRight:10}}>
                      ...
                      <div style={{fontSize:9,color:T.textDim,marginTop:3}}>
                        {item.type} · {item.section} · {item.genre} · {item.hours}h content · {p.percentComplete}%
                      </div>
                    </div>
